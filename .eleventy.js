...
export default function(eleventyConfig) {
    ...
    eleventyConfig.amendLibrary("md", (mdLib) => {
        ...
        const defaultLinkOpen = mdLib.renderer.rules.link_open || function(tokens, idx, options, env, self) {
        ...
        mdLib.renderer.rules.link_open = function(tokens, idx, options, env, self) {
            ...
            if (libdocUtils.isRawLink(linkText, href)) {
                token.attrJoin("class", "raw-link");
            }
    ...
    eleventyConfig.addTransform("raw-links", function(content) {
        if (!this.page.outputPath?.endsWith(".html")) return content;
        return content.replace(/<a\b([^>]*?)>(.*?)<\/a>/gi, (match, attrs, innerText) => {
            if (/<(img|picture|svg|span\s+class="icon-)/i.test(innerText)) return match;
            const plainText = innerText.replace(/<[^>]*>/g, "").trim();
            const hrefMatch = attrs.match(/href=(["'])(.*?)\1/i);
            const href = hrefMatch ? hrefMatch[2] : "";
            if (libdocUtils.isRawLink(plainText, href)) {
                if (/class=(["'])(.*?)\1/i.test(attrs)) {
                    if (!/\braw-link\b/.test(attrs)) {
                        attrs = attrs.replace(/class=(["'])(.*?)\1/i, 'class="$2 raw-link"');
                    }
                } else {
                    attrs = `${attrs} class="raw-link"`;
                }
    ...
    eleventyConfig.addFilter("getNextPage", libdocFunctions.filters.getNextPage);
    eleventyConfig.addFilter("getPreviousPage", libdocFunctions.filters.getPreviousPage);
    // END FILTERS
