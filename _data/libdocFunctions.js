...
const { findNavigationEntries } = require("@11ty/eleventy-navigation/eleventy-navigation.js");
...
export default {
    ...
    filters: {
        ...
        getNextPage: function (collectionsAll, currentUrl) {
            if (!collectionsAll || !Array.isArray(collectionsAll) || !currentUrl) {
                return null;
            }

            const normalize = (u) => (u ? u.replace(/index\.html$/, "").replace(/\/+$/, "") || "/" : "");
            const currentNorm = normalize(currentUrl);

            let homeEntry = null;
            const homePage = collectionsAll.find(
                item => item.url === "/" || item.url === "/index.html" || item.data?.permalink === "index.html"
            );
            if (homePage) {
                homeEntry = {
                    url: homePage.url || "/",
                    title: homePage.data?.title || "Modern HTML & CSS Guide for Artists",
                    key: "Home"
                };
            }

            const navEntries = findNavigationEntries(collectionsAll);

            function flattenTree(nodes) {
                let list = [];
                for (const node of nodes) {
                    const pageItem = collectionsAll.find(
                        p => p.url === node.url || (node.data?.page?.url && p.url === node.data.page.url)
                    );
                    const title = pageItem?.data?.title || node.title || node.key;
                    list.push({
                        url: node.url,
                        key: node.key,
                        title: title,
                        parent: node.parent || node.parentKey || null,
                    });
            ...
            const flatNav = flattenTree(navEntries);
            const readingList = homeEntry ? [homeEntry, ...flatNav] : flatNav;

            const currentIndex = readingList.findIndex(item => normalize(item.url) === currentNorm);
            if (currentIndex >= 0 && currentIndex < readingList.length - 1) {
                return readingList[currentIndex + 1];
            }

            return null;
        },
        getPreviousPage: function (collectionsAll, currentUrl) {
            if (!collectionsAll || !Array.isArray(collectionsAll) || !currentUrl) {
                return null;
            }
