...
} = require("@11ty/eleventy-navigation/eleventy-navigation.js");
    ...
    getNextPage: function (collectionsAll, currentUrl) {
      ...
      const homePage = collectionsAll.find(
        (item) =>
          item.url === "/" ||
          item.url === "/index.html" ||
          item.data?.permalink === "index.html",
      );
      if (homePage) {
        homeEntry = {
          url: homePage.url || "/",
      ...
      function flattenTree(nodes) {
        ...
        for (const node of nodes) {
          const pageItem = collectionsAll.find(
            (p) =>
              p.url === node.url ||
              (node.data?.page?.url && p.url === node.data.page.url),
          );
      ...
      const currentIndex = readingList.findIndex(
        (item) => normalize(item.url) === currentNorm,
      );
