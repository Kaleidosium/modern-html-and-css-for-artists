const sandbox = {
  ...
  resize: function () {
    ...
    if (preWidth > 0 && iframeWidth > 0) {
      ...
      sandbox._resizerEls.monitorIframeHeight.innerHTML =
        sandbox._resizerEls.iframe.querySelector("iframe").clientHeight;
    }
  },
  reset: function () {
    document
      .querySelectorAll(
        `${sandbox._selectors.codeWrapper}, ${sandbox._selectors.iframeWrapper}`,
      )
      .forEach(function (el) {
        el.style.width = null;
      });
  ...
  updateMonitor: function () {
    document
      .querySelectorAll(sandbox._selectors.sandbox)
      .forEach(function (elSandbox) {
        elSandbox
          .querySelectorAll(
            `${sandbox._selectors.codeWrapper}, ${sandbox._selectors.iframeWrapper}`,
          )
        ...
        elSandbox.querySelector(
          sandbox._selectors.monitorIframeWidth,
        ).innerHTML = elIframe.offsetWidth;
        elSandbox.querySelector(
          sandbox._selectors.monitorIframeHeight,
        ).innerHTML = elIframe.offsetHeight;
      });
  },
  handlers: {
    _mousedownResizer: function (evt) {
      ...
      sandbox._resizerEls.sandboxWrapper = evt.target.closest(
        sandbox._selectors.sandbox,
      );
      sandbox._resizerEls.pre =
        sandbox._resizerEls.sandboxWrapper.querySelector(
          sandbox._selectors.codeWrapper,
        );
      sandbox._resizerEls.iframe =
        sandbox._resizerEls.sandboxWrapper.querySelector(
          sandbox._selectors.iframeWrapper,
        );
      sandbox._resizerEls.monitorIframeWidth =
        sandbox._resizerEls.sandboxWrapper.querySelector(
          sandbox._selectors.monitorIframeWidth,
        );
      sandbox._resizerEls.monitorIframeHeight =
        sandbox._resizerEls.sandboxWrapper.querySelector(
          sandbox._selectors.monitorIframeHeight,
        );
      if (
        typeof sandbox._resizerEls.btn == "object" &&
    ...
    _stopResizer: function (evt) {
      ...
      window.removeEventListener(
        "mousemove",
        sandbox.handlers._mousemoveWindow,
      );
    },
    ...
    _clickCopyCode: function (evt) {
      ...
      const code = evt.target
        .closest(sandbox._selectors.sandbox)
        .querySelector(sandbox._selectors.code).innerText;
    ...
    _clickCopyUrl: function (evt) {
      const elBtn = evt.target.closest("button");
      const permalink = evt.target
        .closest(sandbox._selectors.sandbox)
        .querySelector(sandbox._selectors.permalink).href;
      elBtn.classList.add("pe-none");
    ...
    _clickReload: function (evt) {
      const elBtn = evt.target.closest("button");
      const elIframe = evt.target
        .closest(sandbox._selectors.sandbox)
        .querySelector(sandbox._selectors.iframe);
