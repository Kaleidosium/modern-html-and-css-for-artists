const libdocUi = {
  ...
  el: {
    ...
    navSmallDevicesFTOCBtn: document.querySelector(
    ...
    inputsColorScheme: document.querySelectorAll(
  ...
  getCurrentScreenSizeName: function () {
    ...
    Object.keys(libdocUi.defaults.screenSizes).forEach(function (name) {
      ...
      if (
        window.innerWidth >= currentSizeName[0] &&
        window.innerWidth <= currentSizeName[1]
      ) {
        response = name;
      }
    });
  ...
  saveLocalStorage: function ({ identifier, backup }) {
    if (
      libdocUi.localStorageAvailable() &&
  ...
  copyToClipboard: function (textToCopy, options) {
    const params = {
      ...
      notificationContent: function (value) {
        return typeof value == "string"
          ? value
          : libdocMessages.copyToClipboard;
      },
    };
    const userParams = {
    ...
    if (navigator.clipboard !== undefined) {
      navigator.clipboard.writeText(textToCopy).then(function () {
        ...
        if (userParams.notificationEnabled)
          libdocUi.notifications.add(userParams.notificationContent);
      });
    } else {
  ...
  notifications: {
    ...
    add: function (message, options) {
      ...
      const n_markup = this.templates[n_tpl]({
        message: message,
        id: n_id,
        duration: n_duration,
        skin: n_skin,
  ...
  handlers: {
    ...
    _clickCopyCodeBlock: function (evt) {
      ...
      if (elBtn.dataset.originalText === undefined)
        elBtn.dataset.originalText = elBtn.innerText;
    ...
    _scrollNavPrimary: function () {
      const previous =
          libdocUi.handlers._scrollNavPrimaryPreviousPreferenceValue,
    ...
    _windowLoad: function () {
      const textQuery =
        libdocUi._searchParams.get("text") ||
        libdocUi._searchParams.get("search");
      if (textQuery !== null) {
    ...
    _clickSearchInputClear: function (evt) {
      ...
      if (elClearBtn !== null) {
        ...
        elClearBtn.form
          .querySelector('input[type="search"][name="search"]')
          .focus();
      }
    },
  ...
  updateTransferSizeDisplay: function () {
    ...
    if (currentTransferSize > 0) {
      document
        .querySelectorAll(".page-transfer-size-display")
        .forEach(function (el) {
  ...
  getVisibleTocIndexes: function () {
    ...
    if (typeof libdocUi.el.ftocLinks == "object") {
      libdocUi.el.ftocLinks.forEach(function (elLink, linkIndex) {
        const headingRects =
            libdocUi.el.ftocHeadings[linkIndex].getBoundingClientRect(),
          nextIndex = linkIndex + 1,
          elNextHeading = libdocUi.el.ftocHeadings[nextIndex];
        let nextHeadingRects = 0;
        if (elNextHeading !== undefined)
          nextHeadingRects = elNextHeading.getBoundingClientRect();
        if (
          (headingRects.y >= -10 && headingRects.y < window.innerHeight - 50) ||
          (headingRects.y < -10 &&
            nextHeadingRects.y >= window.innerHeight - 50) ||
  ...
  createFloatingToc: function () {
    if (libdocUi.el.ftoc === undefined && libdocUi.el.tocMainOl !== null) {
      ...
      libdocUi.el.ftocList = libdocUi.el.ftoc.querySelector(
        "#floating_toc__list",
      );
      libdocUi.el.navSmallDevicesFTOCBtn.disabled = false;
      libdocUi.el.navSmallDevicesFTOCBtn.addEventListener(
        "click",
        libdocUi.toggleFtocSmallDevices,
      );
      libdocUi.el.navSmallDevicesFTOCBtn.addEventListener(
        "click",
        libdocUi.updateSearchOccurrenceCmdBottom,
      );
      elDetails.addEventListener(
        "toggle",
        libdocUi.handlers._toggleFtocLargeDevices,
      );
      if (libdocUi.getUserPreferences().FTOCNormallyOpened) {
        elDetails.open = true;
        if (
          libdocUi._currentScreenSizeName == "xs" ||
  ...
  createGoToTop: function () {
    if (libdocUi.el.gtt === undefined) {
      ...
      libdocUi.el.gtt.setAttribute(
        "class",
        "d-none--xs d-none--sm | pos-fixed z-2 bottom-0 | p-0 h-50px ar-square mb-5 | fs-6 | brad-4 bc-neutral-100 bwidth-1 bstyle-dashed bcolor-neutral-500 cur-pointer __hover-1 __soft-shadow",
      );
  ...
  updateNavPrimary: function () {
    const userPreferences = libdocUi.getLocalStorage(
      libdocUi.defaults.localStorageIdentifier,
    );
    if (userPreferences?.navPrimaryScrollTop) {
      libdocUi.el.navPrimary.scroll({
        top: userPreferences?.navPrimaryScrollTop,
      });
    } else {
      // Scroll to [aria-current="page"] element
      const elCurrentPageLink = libdocUi.el.navPrimary.querySelector(
          '[aria-current="page"]',
        ),
        elNavP = libdocUi.el.navPrimary;
      if (elCurrentPageLink !== null && elNavP !== null) {
        if (
          elCurrentPageLink.clientHeight + elCurrentPageLink.offsetTop >
          elNavP.clientHeight
        ) {
          elNavP.scroll({
  ...
  getUserPreferences: function () {
    return (
      libdocUi.getLocalStorage(libdocUi.defaults.localStorageIdentifier) || {}
    );
  },
  updateUserPreferences: function (newPreferences) {
    if (typeof newPreferences == "object" && newPreferences !== null) {
      const currentUserPreference = libdocUi.getLocalStorage(
        libdocUi.defaults.localStorageIdentifier,
      );
  ...
  updateFtocList: function () {
    if (
      window.scrollY > libdocUi.el.mainHeader.clientHeight &&
      typeof libdocUi.el.ftoc == "object"
    ) {
      libdocUi.el.ftoc.style.display = null;
      if (libdocUi.el.ftocDetails.open) {
        ...
        if (firstTrueIndex > -1) {
          ...
          if (libdocUi._currentScreenSizeName == "md") {
            if (
              elFirstLink.offsetTop + elFirstLink.clientHeight >
                libdocUi.el.ftocList.scrollTop &&
              elFirstLink.offsetTop + elFirstLink.clientHeight <
                libdocUi.el.ftocList.scrollTop +
                  libdocUi.el.ftocList.clientHeight
            ) {
            } else {
              libdocUi.el.ftocList.scroll({
                top: libdocUi.el.ftocLinks[firstTrueIndex].offsetTop - 80,
              });
            }
          } else {
            if (
              elFirstLink.offsetLeft + elFirstLink.clientWidth / 2 >
                libdocUi.el.ftocList.scrollLeft &&
              elFirstLink.offsetLeft + elFirstLink.clientWidth <
                libdocUi.el.ftocList.scrollLeft +
                  libdocUi.el.ftocList.clientWidth
            ) {
            } else {
              libdocUi.el.ftocList.scroll({
                left: libdocUi.el.ftocLinks[firstTrueIndex].offsetLeft - 10,
              });
            }
          }
        }
  ...
  updateSearchOccurrenceCmdBottom: function () {
    if (libdocUi.el.searchOccurrencesCmd !== undefined) {
      ...
      if (
        ...
        libdocUi._so.bottomSpacing =
          libdocUi.el.navSmallDevices.clientHeight + fTocHeight;
      }
  ...
  createSearchOccurencesCmd: function () {
    if (
      typeof libdocUi.el.main == "object" &&
      ...
      libdocUi.el.searchOccurrencesCmd.setAttribute(
        "class",
        "d-flex fw-wrap jc-space-between ai-center gap-5 | pos-sticky bottom-0 z-1 | pb-5 pt-5 | bs-1 bc-success-100 bradtl-3 bradtr-3",
      );
      ...
      libdocUi.el.searchOccurrencesCurrentIndexPosition =
        document.querySelector("#current_query_occurrence_index_position");
    }
  },
  ...
  searchContent: function (query) {
    if (typeof query == "string") {
      ...
      document
        .querySelectorAll(
          "main>*:not(pre):not(aside):not(header), main>*:not(pre):not(aside):not(header) *, .page_tag",
        )
        .forEach(function (el) {
          const textContentWoChildren =
            libdocUi.getTextContentWithoutChildNodes(el);
      ...
      document
        .querySelectorAll("main>pre, main>.sandbox")
        .forEach(function (el) {
      ...
      if (libdocUi.defaults.colorSchemes.includes(name)) {
        if (name == "light") {
          ...
          libdocUi.el.darkModeCssMetaLink.media =
            libdocUi.defaults.darkModeCssMedia;
        } else if (name == "dark") {
          libdocUi.el.darkModeCssMetaLink.href =
            libdocUi.defaults.darkModeCssFilePath;
          libdocUi.el.darkModeCssMetaLink.media = "";
        } else if (name == "auto") {
          libdocUi.el.darkModeCssMetaLink.href =
            libdocUi.defaults.darkModeCssFilePath;
  ...
  createCopyCodeOnCodeBlocks: function () {
    ...
    if (elsPre.length > 0) {
      elsPre.forEach(function (elPre) {
        ...
        if (elCode !== null) {
          ...
          if (className !== null) {
            ...
            if (languageClassSplit.length > 0)
              elCode.dataset.languageName = languageClassSplit[0]
                .toString()
                .replace("language-", "");
          }
        }
      ...
      const languagesNamesArray = libdocUi.getJson(
        libdocUi.defaults.supportedLanguagesJsonPath,
      );
      languagesNamesArray.then((languagesArray) => {
        if (languagesArray.length > 0) {
          document
            .querySelectorAll("code[data-language-name]")
  ...
  addExternalLinkIconIntoMainContent: function () {
    libdocUi.el.main
      .querySelectorAll('main a[href^="https://"]')
      .forEach(function (el) {
  ...
  updateCustomLinks: function () {
    if (
      libdocUi.el.customLinks !== null &&
      libdocUi.el.navPrimaryContainer !== null
    ) {
      const ctnWidth = libdocUi.el.navPrimaryContainer.clientWidth,
        elsLinks = libdocUi.el.customLinks.querySelectorAll("a"),
        isLargeScreen =
          libdocUi.getCurrentScreenSizeName() == "md" ? true : false;
      if (
        elsLinks.length > 1 &&
        isLargeScreen &&
        libdocUi._updateCustomLinks === null
      ) {
        ...
        const menuItems = [],
          paddingLeft = parseFloat(
            getComputedStyle(libdocUi.el.customLinks).paddingLeft,
          ),
        ...
        libdocUi._updateCustomLinks.items.forEach(function (item) {
          ...
          if (tempWidth > threshold) {
            elLink.setAttribute(
              "class",
              "d-flex ai-center gap-1 | pt-3 pb-3 p-5 | fvs-wght-700 fs-2 lsp-3 lh-1 tt-uppercase td-none ws-nowrap | c-primary-600",
            );
        ...
        if (menuItems.length > 0) {
          ...
          elMenuItemsCtn.setAttribute(
            "class",
            "d-flex fd-column | pos-absolute right-0 | mr-3 | bc-neutral-100 bwidth-1 bstyle-dashed bcolor-neutral-500 brad-2 __soft-shadow",
          );
          elDetails.setAttribute("class", "pos-absolute right-0");
          elSummary.setAttribute(
            "class",
            "d-flex ai-center jc-end gap-1 | pt-2 pb-2 pl-4 pr-4 | lh-1 | cur-pointer c-primary-600",
          );
          elSummary.setAttribute("title", libdocMessages.otherCustomLinks);
          elSummary.innerHTML =
            '<span class="icons"><span class="icon-plus-circle"></span><span class="icon-minus-circle"></span></span>';
          elDetails.appendChild(elSummary);
  ...
  update: function () {
    ...
    libdocUi.updateCustomLinks();
    libdocUi.el.navPrimaryCheckbox.addEventListener(
      "change",
    ...
    libdocUi.el.navPrimary.addEventListener(
      "scroll",
      libdocUi.handlers._scrollNavPrimary,
    );
    ...
    libdocUi.el.searchClearBtns.forEach(function (elClearBtn) {
      elClearBtn.addEventListener(
        "click",
        libdocUi.handlers._clickSearchInputClear,
      );
    });
    document
      .querySelectorAll('details[name="nav_primary"]')
      .forEach(function (elDetail) {
        elDetail.addEventListener(
          "toggle",
          libdocUi.handlers._toggleNavPrimaryAccordion,
        );
      });
    ...
    document.addEventListener(
      "DOMContentLoaded",
      libdocUi.handlers._DOMContentLoaded,
    );
  },
};
libdocUi.update();
