...
const cToggle = {
  ...
  documentClick: function (event) {
    ...
    const el_closest_toggle = event.target.closest(
      "[c-toggle], [c-toggle-name]",
    );
    if (el_closest_toggle === null) {
      ...
      const close_children_ids =
        el_closest_toggle.dataset.onclickForceDismissChildrenIds;
  ...
  open: function (id) {
    ...
    // Create custom event
    const event = new CustomEvent(`cToggle_event`, {
      detail: { id: id, method: "open" },
    });
  ...
  update: function () {
    ...
    document.querySelectorAll("[c-toggle]").forEach(function (el_trigger) {
      ...
      const els_targets = document.querySelectorAll(
        '[c-toggle-name="' + toggle_id + '"]',
      );
      ...
      els_targets.forEach(function (el_target) {
        ...
        if (el_target.dataset.classOrigin === undefined)
      ...
      if (els_targets.length > 0) {
        ...
        if (event_name !== undefined) {
          ...
          if (event_name == "mousehover" || event_name == "mouseover") {
            el_trigger.addEventListener(
              "mouseenter",
              cToggle.handlers.mouseenter,
            );
            el_trigger.addEventListener(
              "mouseleave",
              cToggle.handlers.mouseleave,
            );
