const fuzzy = {
        ...
        fuzzy.autocomplete[inputId] = new Autocomplete(inputId, {
          ...
          onResults: ({ matches }) => {
            return matches
              .map((el) => {
                let description = "";
                if (el.item.description.trim().length > 0)
                  description = `<span>${el.item.description}</span>`;
  ...
  initIndex: function (evt) {
    if (
      fuzzy.indexItems === null &&
      typeof libdocSystem.fuzzyIndexUrl == "string"
    ) {
        ...
        if (!filterArray.includes(item.title)) {
          ...
          switch (item.title) {
            case "./core/libdoc_blog.liquid":
              ...
              item.description =
                libdocConfig.blogDescription === "false"
                  ? ""
                  : libdocConfig.blogDescription;
  ...
  update: function () {
    if (
      ...
      typeof libdocSystem.searchIndexUrl == "string"
    ) {
      if (fuzzy.el.searchInput !== null) {
        fuzzy.el.searchInput.addEventListener(
          "focus",
          fuzzy.handlers._focusInput,
        );
      }
      if (fuzzy.el.searchInputAlt !== null) {
        fuzzy.el.searchInputAlt.addEventListener(
          "focus",
          fuzzy.handlers._focusInput,
        );
      }
